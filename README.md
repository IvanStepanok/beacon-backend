...
- One contract, two clients. The `Report` JSON is a superset: nested objects
  (mobile) plus flat aliases (dashboard) coexist, so a single document deserializes
...
- Open standards and interoperability. PostGIS geometry; export to GeoJSON,
...
- Built to scale and not fall over. pgxpool, SQL-side aggregations (the overview
...
- Privacy and governance. Anonymous submitters, anonymization flags stored per
